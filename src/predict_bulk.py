...
def predict_volume(model, image_data, device, desired_size=(256, 256), threshold=0.5):
    ...
    for i in range(D):
        ...
        # Resize each slice to desired_size
...
def main():
    ...
    OUTPUT_DIR = 'outputs/predictions/Unet-3ch-voxel-box/bulk/'
    ...
    for patient_name, image_path in bulk_images:
        ...
        # Load and resample image to target_spacing
    ...
    if num_patients < 15:
        print(f"Warning: Expected 15 patients, but got {num_patients}. We'll only plot the ones we have.")
